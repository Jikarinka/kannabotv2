    ...
    let sock = Object.defineProperties(conn, {
        ...
        sendFile: {
            ...
            async value(jid, path, filename = '', caption = '', quoted, ptt = false, options = {}) {
                ...
                if (fileSize >= 300) throw new Error('File size is too big!')
                let opt = {}
...
END:VCARD
        ...
        reply: {
            ...
            async value(jid, text = '', quoted, options){
                if (Buffer.isBuffer(text)) return conn.sendFile(jid, text, '', options.text || '', quoted, options.ptt || 0, options)
                let msg, file, jpg, thum, title, body, desc, media, source, type, render, ads, expi, men, thumUrl = 'https://telegra.ph?id=' + moment(new Date * 1).tz('Asia/Jakarta').format('DD/MM/YYYY/HH:mm:ss') + (Math.random(1234567890987654321) * 1234567890987654321)
                if (options) {
                    thum = options.thum || options.thumb || options.thumbnail || ''
                    title = options.title || ''
                    body = options.body || ''
                    desc = options.desc || ''
                    media = options.med || options.media || ''
                    source = options.sourc || options.source || ''
                    type = options.type || options.medType || options.mediaType || ''
                    men = options.mention || options.mentions || options.contextInfo?.mentionedJid || ''
                    render = options.render || options.larger || options.large || options.largerThumb|| false
                    ads = options.ads || options.iklan || false
                    expi = options.ephe || options.ephemeral || options.expi || options.expiration || ''  
                    if (thum) {
                          thum = await conn.getFile(thum)
                          thum = thum.data
                    }
                    if (thum && render) {
                        msg = { 
                            text, 
                            contextInfo: {
                                mentionedJid: Array.isArray(men) ? men : [men], 
                                externalAdReply: {
                                    title: title, 
                                    body: body,			 
                                    sourceUrl: source, 		
 ...
 async value(text, who, m, chatupdate) {
        ...
        editMessage: {
        	async value(key, text, opts = {}) {
        		if (!key || typeof key !== "object") return null;
        		await conn.relayMessage(key.remoteJid, {
        			protocolMessage: {
        				key,
        				type: 14,
        				editedMessage: {
        ...
        sendButton: {
            ...
            async value(jid, text = '', footer = '', buffer, buttons, quoted, options) {
                ...
                let message = {
                    ...
                    footer,
                    mentions: conn.parseMention(text + footer),
        ...
        copyLink: {
        async value(jid, text = "Example", footer = "", copy = "https://example.com/", copyButton = "Copy") {
            let HTB = {
              urlButton: {
                displayText: copyButton,
            ...
            let id = await conn.relayMessage(jid, {
              viewOnceMessage: {
                message: {
                  templateMessage: T
                }
    ...
    async value(jid, title, text, footer, buttonText, buffer, listSections, quoted, options) {
        ...
        sendListM2: {
    async value(jid, title, text, footer, buttonText, titleRow, rows, quoted, options = {}) {
