...
let handler = async (m, { conn, usedPrefix: _p, __dirname, args, Func, usedPrefix }) => {
	let tags
  ...
  let arrayMenuEnter = arrayMenu.map((item) => '┊•  ' + item).join('\n');
...
® ${pmenus} OWNER ADALAH DEWA`, rowId: ".donasi", description: `${namebot}`},
    ...
    {title: `🎰 ${pmenus} Maxer`, rowId: ".? nulis", description: "Cetak Sesuatu Yang Kalian Pilih"},
...
if (teks == '404') {
  let newthumb = "https://telegra.ph/file/3c94e084ddbf2def52f55.jpg"
  return conn.reply(m.chat, Func.example(usedPrefix, m, `\n❏═┅═━–〈 *C A T E G O R Y*\n${arrayMenuEnter}\n┗––––––––––✦`), 0, {
      contextInfo: {
        mentionedJid: [m.sender],
        externalAdReply: {
          title: `${htjava} KANNA BOT`,
          body: titlebot,
          ...
          thumbnail: await (await fetch(newthumb)).buffer(),
 ...
 let mode = global.opts['self'] ? 'Private' : 'Publik'
  ...
  let fkon = { key: { fromMe: false, participant: `${m.sender.split`@`[0]}@s.whatsapp.net`, ...(m.chat ? { remoteJid: '16504228206@s.whatsapp.net' } : {}) }, message: { contactMessage: { displayName: `${name}`, vcard: `BEGIN:VCARD\nVERSION:3.0\nN:;a,;;;\nFN:${name}\nitem1.TEL;waid=${m.sender.split('@')[0]}:${m.sender.split('@')[0]}\nitem1.X-ABLabel:Ponsel\nEND:VCARD`}}}
    ...
    //------------------ SIMPLE
    await conn.reply(m.chat, text, 0, {
      contextInfo: {
        mentionedJid: [m.sender],
        externalAdReply: {
          title: `${htjava} KANNA BOT`,
...
//-------DOC TEMPLATE
    ...
    //------------------- BUTTON VID
    // conn.sendButton(m.chat, text, wm, 'https://telegra.ph/file/a46ab7fa39338b1f54d5a.mp4', [['Ping', '.ping'],['Owner', '.owner'],['Donasi', '.donasi']],ftoko, { gifPlayback: true, contextInfo: { externalAdReply: {title: namebot, mediaUrl: thumb, renderLargerThumbnail: true, body: bottime, sourceUrl: sig, thumbnail: await (await fetch(thumb)).buffer() }}})
