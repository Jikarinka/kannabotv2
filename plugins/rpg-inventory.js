...
const img = "https://telegra.ph/file/c7e01d71067fb7fe7e481.jpg";
...
*───── ɪᴛᴇᴍs ─────*
${items}
➔ ᴛᴏᴛᴀʟ ɪᴛᴇᴍs: ${Object.keys(inventory.items)
          .map((v) => user[v])
...
*───── ᴄʀᴀᴛᴇs ─────*
${crates}
➔ ᴛᴏᴛᴀʟ ᴄʀᴀᴛᴇs: ${Object.keys(inventory.crates)
          .map((v) => user[v])
          .reduce((a, b) => a + b, 0)} Crates`
      : ""
  }${
    pets || user.petFood
      ? `
...
*───── ᴘᴇᴛs ─────*
${pets ? pets + "\n" : ""}${user.petFood ? "🍖 ᴘᴇᴛғᴏᴏᴅ: " + user.petFood : ""}`
      : ""
  }${
    cooldowns
      ? `
...
`.trim();
  ...
  //   [
  //     [
  //       `${user.health < 60 ? "ʜᴇᴀʟ" : "ᴀᴅᴠᴇɴᴛᴜʀᴇ"}`,
  //       `${user.health < 60 ? ".heal" : ".adventure"}`,
  //     ],
  //     ["ᴘʀᴏғɪʟᴇ", ".pp"],
  //   ],
  //   m
  ...
  await conn.reply(m.chat, caption, 0, {
    contextInfo: {
      ...
      externalAdReply: {
        title: `${htki} ɪɴᴠᴇɴᴛᴏʀʏ ${htka}`,
        body: `🧑🏻‍🏫  ɴᴀᴍᴇ: ${conn.getName(m.sender)}`,
